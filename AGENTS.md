...
This file is read by Codex at the start of every session. Follow every instruction here exactly. Do not skip sections. Do not deviate from the rules unless the user explicitly overrides them in the prompt.
...
2. Paste it in your prompt to Codex
3. Codex will call `get_design_context` and `get_screenshot` automatically to extract layout, spacing, colors, and component specs before writing any code
...
```
/
├── AGENTS.md                  ← You are here. The source of truth.
...
| ScheduleDemoPanel | components/ScheduleDemoPanel/ScheduleDemoPanel.tsx | open, onClose, onScheduled?(date, time) — 720px right drawer (same shell as FormDrawer: dimmed backdrop, back-arrow header) for booking a "Schedule a call with a Birdeye specialist" demo; self-contained month calendar (Mon-start grid) + scrollable time-slot list, footer "Schedule call" CTA. Opened from the Empty-state "Schedule demo" CTA on Overview v3's AI workforce summary banner |
...
Check AGENTS.md Section 5 (Component Registry) before writing anything.
List which existing components you will reuse and which new ones you need to create.
Then build any missing components first, register them in AGENTS.md Section 5, then compose the screen.
```
...
Follow the file structure in AGENTS.md Section 6.
...
This project is built one task per conversation. **Start a fresh chat (or `/clear`) for each new task** — AGENTS.md reloads automatically, so project context is never lost. Do not carry one long thread across many tasks (the whole history is re-billed every prompt).
...
This section tells Codex (and teammates) exactly how to take a new PRD and wire it into the MYNA prototype as a working agent workflow.
