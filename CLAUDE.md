...
| GhostwriterKnowledgeTab | components/GhostwriterKnowledgeTab/GhostwriterKnowledgeTab.tsx | no props — the **Knowledge tab** of the same Ghostwriter create-flow shell. Static/read-only mock: a bordered list of knowledge docs (icon, title, "Updated X ago" / source meta, right-aligned "Edit" text button), plus a trailing "+ Add knowledge" pill button. Hardcoded array in the file, not wired to a real knowledge store |
| GhostwriterRunTestModal | components/GhostwriterRunTestModal/GhostwriterRunTestModal.tsx | open, reviews (`Review[]` from `data/reviewsData.ts`), selectedIds (string[]), onToggleReview(id), onCancel, onRunTest — **Jay & Robin only**: the "Run test" review picker opened from the Test tab's empty state. `createPortal`'d 1100px centered modal ("Preview settings" + subtitle, ✕): left scrollable multi-select list (each row a checkmark + full `ReviewCardBody`, click-to-toggle, no radio/single-select), right a static "Filter by" column (Time period/Review source/Ratings/Review content — visual only, not wired); footer "N reviews selected" + Cancel / Run test (disabled until ≥1 picked). Portalled to `<body>` for the same reason `GhostwriterSimulation`'s panels are — the Ghostwriter shell's tab bar is a pinned `z-30` |
| GhostwriterTestRunPanel | components/GhostwriterTestRunPanel/GhostwriterTestRunPanel.tsx | reviews (`Review[]`), centerContent (ReactNode — caller mounts the canvas), testedAt? (pre-formatted string, e.g. "Sep 22, 2026, 3:45 PM"), className? — **Jay & Robin only**: the Test tab's result view after "Run test" is confirmed. Three columns: left a list of the picked reviews (reviewer + `StarRating` + snippet + green check, click to select) with `testedAt` under the "N reviews tested" count; center `centerContent` unchanged (the caller passes a second `WorkflowEditorScreen` mount showing the same built workflow); right a 420px detail panel — `Chip variant="success"` "Passed", then the workflow run itself (reusing `TestRunPanel`'s exported `TestRunStepRow` + the `useTestRun` hook over `buildTestRunSteps(REVIEW_RESPONSE_WORKFLOW...)`, same stepper the canvas's own "Run test" uses, keyed by review id so switching reviews restarts it), then the selected review's full `ReviewCardBody` with its existing simulated reply from `data/reviewsData.ts`. Owns its own `selectedId` local state, defaulting to the first review |
