...
| AppointmentWidgetEditorScreen | screens/AppointmentWidgetEditorScreen.tsx | widget ({name,location,createdBy,createdOn}), onBack, onOpenBookingTemplates?(templateId) — Settings → Widgets → Appointment widgets → row click/Edit opens this. Breadcrumb (Settings/Widgets/Appointments/name) + header (title+pencil, location link, gear/Cancel/Save) + Install/Appearance/Configuration/Preferences tabs (only Configuration has real content), matching the real product's two-column layout: LEFT is one continuous divided card — Widget title and description → Booking template (`BookingTemplateSelectField`) → "Allow customers to choose service providers at unintegrated locations" toggle → Form fields (read-only `BaseFieldsEditor` summary of the selected template, ends in a "Customise form" link that opens the template editor — no inline field builder here anymore) → Booking preferences (inline "show slots up to N months" / "minimum notice N day(s)" selects); RIGHT is a sticky live Preview card (desktop/mobile toggle + expand icon, reflects the typed booking header/description, static calendar mock) |
