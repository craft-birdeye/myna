...
| NodesUpdatedCard | components/JayRobinCreateFlow/NodesUpdatedCard.tsx | nodes (`JrNodeUpdate[]` — `{kind:'added'\|'changed', icon, label}`), onAccept, onUndo, className? — **Jay & Robin only**: the "N nodes updated · Accept / Undo" review card `JayRobinCreateFlow` docks above the composer once the build pass finishes. Collapsed: chevron + count + the two actions (primary Accept, secondary Undo); expanded: one row per node with a green `+` (added) or grey `±` (rewired Start/End) marker and the node's glyph. Purely presentational — the flow decides what Accept/Undo say next |
