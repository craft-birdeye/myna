...
| EstimateSavingsModal | components/EstimateSavingsModal/EstimateSavingsModal.tsx | open, values (`EstimateSavingsValues`: mode 'time'\|'cost', timePerUnitMins, currency, hourlyWage), unitLabel (e.g. 'session'\|'conversation'), onClose, onSave — centered 640px "Estimate savings" modal (Time saved/Cost saved radio, "Time saved per {unitLabel}" mins field, Cost mode adds a currency+hourly-wage field); opened via `SummaryStats`' per-stat `onConfigure` tune-icon button. Used by `HCFrontdeskOverview2Screen`'s Summary row to make the "Time saved"/"Cost saved" tile user-configurable |
...
| ViewModeToggle | components/ViewModeToggle/ViewModeToggle.tsx | value ('conversations'\|'sessions'), onChange — Sessions/Conversations segmented toggle (Sessions first + default) with per-option hover tooltip explaining the term; also exports `NOUN_FORMS` (label forms per mode) and `scaleForViewMode(value, mode)` (session counts run higher via a shared 1.35x multiplier since one conversation can hold multiple sessions). Used by `HCFrontdeskOverview2Screen`, `AgentDetailScreen` (Agents tab header), and `AgentInstanceScreen` (Outcomes tab header) — reuse this instead of hand-rolling another conversations/sessions switcher |
...
| SummaryStats | components/charts/SummaryStats.tsx | title?, stats[] ({id,value,label,delta?,trend?,onConfigure?}) — KPI row with up/down deltas; `onConfigure` shows a small tune-icon button next to that stat's label (e.g. opening `EstimateSavingsModal`) |
