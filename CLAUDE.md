...
| VoiceCallEngineSettings | components/VoiceCallEngineSettings/VoiceCallEngineSettings.tsx | no props — STT/TTS engine model block: STT Primary model + Failover policy (conditional fallback Model picker shown only when policy='Manual') + Enable interruptions toggle, then TTS Primary model; used above Default voice in agent Settings and setup wizard Voice call settings |
| VoiceCallInterruptionSettings | components/VoiceCallEngineSettings/VoiceCallEngineSettings.tsx | no props — TTS Failover policy (same conditional fallback-Model + fallback-Voice pickers); rendered after Default voice/Add additional voice and before Greeting message, sharing the file with `VoiceCallEngineSettings` |
