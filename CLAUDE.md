...
| FrontdeskToolsTab | components/FrontdeskToolsTab/FrontdeskToolsTab.tsx | no props — **Front desk (Sep 23) only**'s own Tools tab, swapped in for `GhostwriterConnectionsTab` inside the same `createGhostwriterTab === 'tools'` branch in `AgentDetailScreen` (`isFrontdeskSep23Polish ? <FrontdeskToolsTab /> : <GhostwriterConnectionsTab />` — Myna keeps `GhostwriterConnectionsTab` unchanged). Two sub-`Tabs` ("Internal tools"/"External tools") over a 3-column card grid (not the full-width row list `GhostwriterConnectionsTab` or the workflow canvas's `AddToolDrawer` use) — 9 internal tools (Update contact preferences, Escalate to human, Select message template, Add/Remove contact to/from list, Send internal alert, Assign/Classify tags, Check business hours), every card sharing the same wrench (`build`) glyph, plus a static "All tools" label (no real dropdown — there's only one category); 5 external tools (Salesforce/Freshdesk/QuickBooks Online/ServiceTitan/Zendesk), each a brand-tinted circle glyph (same `glyphClassName` trick `GhostwriterConnectionsTab`'s reach cards use) + a "Connect" link, none pre-connected. All mock data, nothing wired |
