...
| Tooltip     | components/Tooltip/Tooltip.tsx         | content, variant?='detail'\|'brief', children (trigger), className? — hover tooltip per Aero DS "Tooltip / Web" (Figma `xecPAre4cKkeXEdvTig1oI`, node 2180:72): dark `bg-tooltip` (#252525), white `text-small`, `rounded-sm`, `shadow-tooltip`. `brief` = short phrase (`max-w-[140px]`), `detail` = explanatory text (`max-w-[280px]`, prefer ≤2 lines) |
...
| ChatHistorySideNav | components/ChatHistorySideNav/ChatHistorySideNav.tsx | title, chats[] ({id,label}), activeChatId?, onSelectChat?, onNewChat? — 222px L2 panel (same width as `SideNav`) for chat-style flows: module-name title (e.g. "Front desk", not the per-chat name), "Search chats" filter row, "All chats" row with a blue add-circle "new chat" button, collapsible "Recent chats" list. Nothing is selected until a chat exists — the front desk/reminder create-agent screen in `AgentDetailScreen` only prepends a `{id:'current', label}` entry to `chats` (and selects it) once the user actually submits their first prompt, matching the Claude/ChatGPT landing-vs-recorded-chat pattern; the label comes from the chat's own agent-name state, falling back to the "New front desk agent" placeholder title until a name is chosen |
...
| LogDetailsPanel | components/LogDetailsPanel/LogDetailsPanel.tsx | row (HealthcareLogRow), callerNumber?, sidNumber?, languageDetected?, callEndReason?, routedVia?, summary?, transcript?, durationSecs?, audioUrl?, onViewConversation? — conversation details side panel (meta card, collapsible AI summary, call recording via `CallRecordingPlayer`, call transcript) |
| RunDetailsPanel / RunConversationThread | components/RunDetailsPanel/RunDetailsPanel.tsx | `RunDetailsPanel`: onViewConversation?, steps?, conversation? — 600px "Run details" side panel with Logs (step timeline) / Conversation tabs. `RunConversationThread`: entries (`ReminderConversationEntry[]` from `data/reminderInboxConversation.ts`) — renders a run conversation thread (system labels, blue/gray email cards, voice bubbles with LLM/STT meta); reused by `InboxScreen` for the Sarah Lauren reminder deep-link (`REMINDER_INBOX_CONVERSATION_ID`) |
| WorkflowPreviewPanel | components/WorkflowPreviewPanel/WorkflowPreviewPanel.tsx | agentName, product?, expanded?, onExpand?, onClose — view-only workflow preview reusing `WorkflowViewerTab`'s `viewOnly` `AgentBuilder` canvas (no editing, no drag/drop); header has back/expand/close icon buttons matching `ProcedurePreviewPanel`'s convention. `expanded`/`onExpand` are controlled — the parent (`AgentDetailScreen`'s right-side fixed panel wrapper) owns the actual width (480↔800px in-place widen, `AiAssistPanel`-style), this component just renders the toggle. Used for the "View workflow" card in the front desk create-agent chat |
