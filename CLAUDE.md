...
| RunDetailsPanel | components/RunDetailsPanel/RunDetailsPanel.tsx | steps (RunLogStep[] — trigger/task/delay/branch, each with expandable output/tool/inputs field trees), conversation (ReactNode) — side panel opened from a log row's "View log" action, starting directly on its Logs/Conversation tabs (no header row above them). Logs tab renders the run's trigger/task history (nested `RunLogField` chips, "View inputs"); Conversation tab renders whatever `conversation` node the caller passes (e.g. `LogDetailsPanel` puts a `CallRecordingPlayer` + transcript there) |
| LogDetailsPanel | components/LogDetailsPanel/LogDetailsPanel.tsx | row (HealthcareLogRow), agentName?, transcript?, durationSecs?, audioUrl? — call-log flavor of `RunDetailsPanel`: builds this call's trigger/task steps for the Logs tab and puts a `CallRecordingPlayer` + transcript in the Conversation tab. Agent transcript bubbles show thumbs up/down (same `ShareFeedbackModal` + `useFeedbackRecommendationsStore().submitFeedback` flow as `InboxScreen`), tagged with `agentName` so it lands on that instance's Recommendation tab |
