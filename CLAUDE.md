...
| GhostwriterQuestionCard | components/GhostwriterQuestionCard/GhostwriterQuestionCard.tsx | question, hint?, options? (`{id,label,description?,recommended?}[]`), freeText? (`{placeholder,submitLabel}`), index?/total? + onPrev?/onNext? (pager, hidden unless total>1), onPick?(label), onSubmitText?(text), onSkip?, skipLabel?='Skip', onClose?, className? — **the one way the Ghostwriter asks for anything.** An inline card in the conversation, deliberately *not* a modal: a modal covers the reasoning the question came out of and implies the thread is blocked on an answer that is usually optional. Handles both asks in one component so they can't drift — numbered option rows (number badge in `bg-surface-l2`, full-row `hover:bg-surface-hover`, `Chip variant="info"` for Recommended, optional description line) and a free-text row (pencil badge + borderless input) that doubles as the "write something else" escape under options. The free-text row's trailing button is `skipLabel` until something is typed, then `freeText.submitLabel`; Enter submits. ✕ renders only when `onClose`/`onSkip` is given — omit both on a question that must be answered to continue. Used by `ReviewModeChoiceCards` (live-vs-backlog + the playbook's template options), `GhostwriterDigestPrompt` (spam digest email), and `GhostwriterOpenQuestions` (the playbook's clarifying question, one at a time via the pager). **Don't put the question in a paragraph above the card** — the card header already says it |
| JayRobinCreateFlow | components/JayRobinCreateFlow/JayRobinCreateFlow.tsx | prompt, attachments? (`JayRobinFlowAttachment[]` — chips on the opening turn; a `file` switches the flow to the **requirements-doc variant**: `JR_FILE_*` analysis reads the doc, the first question offers to create the two templates it names that the account lacks, step 4 creates them and a Select template node, the closing line names it), onCreateAgent?, onOpenPlan?, planOpen?, agentCreated?, onAnswerCardOpenChange?, onBusyChange?, pendingAnswer?/onPendingAnswerConsumed? (answers the open question — or, once the agent exists, starts a **follow-up**: user turn → Working → one line → `ProposalCard` (title, 2–3 lines, Apply / Not now; nothing changes before Apply) → Working → a reply whose node names are chips), onOpenNode?(nodeId, tool?), onWorkflowExtra?(extra) (`'templates'` on Accept in the file variant, `'ticket'` when the ticket follow-up is applied — `AgentDetailScreen` feeds it to `WorkflowEditorScreen.workflowExtras`, which runs `extendReviewResponseWorkflow` so the node appears on the canvas), onRewindToStart?(prompt) — **Jay & Robin (and 23 Sep) only**: the whole create-agent story as one self-contained component, mounted by `AgentDetailScreen` in place of `ReviewResponseThread` when `simulationTabLabel === 'Test'`. Beats: user prompt → one intro line → `AgentWorkSequence` analysis pass ("Working #s" → "Worked for #s", body-text narration + "Ran N tools" groups of glyph rows on a rail, rows with detail expand via `›`; collapsed by default, opens only on the chevron) → three `GhostwriterQuestionCard`s asked back to back, one docked card at a time (options + "Other…" free text + Skip), each echoed as a user turn → a brief plan card (five numbered pointers built from the answers by `buildJrPlan`, "See details" opens the parent's plan panel, "Create agent") → a second `AgentWorkSequence` with `kind: 'step'` phases executing the pointers one by one (check-row per step, actions underneath, "Your agent is ready." summary) → `NodesUpdatedCard` (Accept / Undo) → closing line with copy/good/bad actions. Every user turn shows time · copy · rewind on hover; rewind truncates the thread back to that turn (the first turn hands off to `onRewindToStart`, which resets the parent flow and refills the landing composer); rewind retires once the agent exists. `AgentRichLine` types a sentence whose `{ nodeId, label }` segments become chips (`Create ticket ›`) that open that node's panel — the name of the thing built is how you reach it. Follow-up scripts are matched by `matchJrFollowUp` (ticket, else a generic fallback); the landing's "+ → Files" on this flow attaches `JR_REQUIREMENTS_ATTACHMENT` and pre-fills `JR_REQUIREMENTS_PROMPT`, and the first composer click after the agent exists pre-fills `JR_TICKET_FOLLOW_UP_PROMPT`. All copy lives in `data/jayRobinCreateFlow.ts`; the extra nodes in `data/reviewResponseBuildReveal.ts` |
| NodesUpdatedCard | components/JayRobinCreateFlow/NodesUpdatedCard.tsx | nodes (`JrNodeUpdate[]` — `{kind:'added'\|'changed', icon, label, id?}`), onAccept, onUndo, onOpenNode?(nodeId), className? — the "N nodes updated · Accept / Undo" review card `JayRobinCreateFlow` docks above the composer once the build pass finishes, and `ReviewCoachingFlow` docks after a coaching fix. Rows with an `id` become buttons when `onOpenNode` is given (chevron at the end) — the parent turns that into `WorkflowEditorScreen`'s `externalOpenNode` so the node's RHS panel opens on the canvas. Collapsed: chevron + count + the two actions (primary Accept, secondary Undo); expanded: one row per node with a green `+` (added) or grey `±` (rewired Start/End) marker and the node's glyph. Purely presentational — the flow decides what Accept/Undo say next |
| SparkleLoader | components/SparkleLoader/SparkleLoader.tsx | size?=18, spinning?=true, className? — the gradient AI sparkle inside the `bg-ai-summary` avatar disc beside every agent turn in the create-flow chats; rotates + pulses while `spinning`. Extracted from `AgentDetailScreen` so `JayRobinCreateFlow` can share it. Pairs with `hooks/useTypewriter.ts` (also extracted from the screen) for typed agent lines |
| ReviewCoachingFlow | components/ReviewCoachingFlow/ReviewCoachingFlow.tsx | item (`ReviewCoachingItem` from `data/reviewCoaching.ts`), onOpenNode?(nodeId), onResolved?('accepted'\|'rejected') — **Review response agents (Jay & Robin, 23 Sep)**: the copilot conversation for one piece of review coaching, docked on the workflow canvas in the `AiBuilderPanel` `content` slot (owns its own scroller + `PromptComposer`). Beats: the feedback as a user turn with the trace (review + the agent's reply) → `AgentWorkSequence` (reads the trace, checks the node that decided) → one-paragraph finding → `GhostwriterQuestionCard` asking how to fix it (options + "Other…"; **nothing changes before the user answers**) → echoed choice + ack → `AgentWorkSequence` apply pass (check-rows) → `NodesUpdatedCard` Accept / Undo (rows open the node) → closing line. Themes (spam / staff name / language / tone / generic) are classified from the feedback text by `classifyReviewCoachingTheme`; every script and copy string lives in `data/reviewCoaching.ts` |
| CoachingCanvasPane | screens/CoachingCanvasPane.tsx | session (`{recommendationId, instanceName}`), onOpenNode?, onClose — what the canvas copilot shows during a coaching session: a review coaching item → `ReviewCoachingFlow` (status written to `RecommendationOverridesStore` on Accept/Undo); a front desk recommendation → `RecommendationDetailScreen layout="docked"` (exact same chat as the old full page, beside the canvas). Mounted by `App` through `WorkflowEditorScreen`'s `aiBuilderPanelContent` with `flushAiPanel` (panel takes the left rail's place, 448px at 16px, title "Coaching"); reached from the Reviews toast's "Track feedback", a Coaching-tab row, or the reply's "Track your feedback" link |
| AgentUnifiedNavScreen | screens/AgentUnifiedNavScreen.tsx | onBack?, onEditWorkflow?, onOpenCoaching?(recommendationId) — **prototype** (nav `response-agents-unified-nav`, "Response agent (unified nav)"): one consistent agent-level navigation for every agent — five pill sections on top (Build · Test · Deploy, gap, Observe · Optimize) and a `Tabs` row of L1s that varies by section, not by agent (Build: Agent composer / Procedure / Tools / Knowledge / Workflows; Test: Test runs / Test suite; Deploy: Channels / Versioning; Observe: Logs / Human review / Analytics; Optimize: Recommendation / Coaching). Shown on "Review response agent - North Region": Workflows mounts `WorkflowViewerTab`, Test runs `GhostwriterTestRunPanel layout="fullpage"`, Logs `AgentLogsTab`, Analytics `MetricTiles`, Coaching `RecommendationsTab` with the review coaching items; every other L1 is a labelled `RowList` placeholder so the nav can be judged on its own. Remembers the last L1 per section |
| GhostwriterSimulation | components/GhostwriterSimulation/GhostwriterSimulation.tsx | active?=true, onNotify?(message), className? — the **Simulation tab** of the Ghostwriter (review response) shell in `AgentDetailScreen`. Empty state offers "Generate tests" (the agent writes one test per rule in `ghostwriterPlan.ts`, plus two edge cases those rules leave open) or "Add test" (a `FormDrawer`: name, the review, rating/source/location selects, expected behaviour). "Run all" walks the rows and marks each Passed/Failed; clicking a not-yet-run row runs just that one, and clicking a run row opens a portalled 600px detail panel — verdict banner, the scenario as a `StarRating` + review card, Expected, a step timeline where `warn` steps are the failure point, and the Outcome (Posted/Held/Assigned/Skipped) with the reply it wrote. Content and timings live in `data/ghostwriterSimulation.ts`; there is no model — it is scripted. **Two things to keep:** (1) `active` hides it with `display:none` rather than unmounting, so generated tests survive a trip to the Workflow tab and back; (2) the detail panel and the drawer are `createPortal`'d to `<body>` because the shell section is a `z-10` stacking context and a plain `fixed` overlay there paints *under* the `z-30` pinned tab bar |
| GhostwriterConnectionsTab | components/GhostwriterConnectionsTab/GhostwriterConnectionsTab.tsx | no props — the **Tools tab** of the Ghostwriter (review response) create-flow shell in `AgentDetailScreen` (Create agent CTA from the agent list view only) — also still used by **Front desk (Myna)**. Static/read-only mock: a "Reach your agents from another app" row of 4 cards (WhatsApp/Slack/iMessage/Telegram, brand icon + description) and a "Data sources" 2-column grid of rows (icon, label, meta, and a `Chip variant="success" showDot` "Connected" pill / blue "Reconnect" / outline "Connect" button per row). All data is a hardcoded array in the file — nothing wired to real integration state yet |
| FrontdeskToolsTab | components/FrontdeskToolsTab/FrontdeskToolsTab.tsx | no props — Front desk (Sep 23) Tools tab. Renders `AgentToolsTab variant="frontdesk"`: a Connected tools section (5 mixed internal/external) and an Add button that opens the catalog side panel. Myna still uses `GhostwriterConnectionsTab` |
| AgentToolsTab | components/AgentToolsTab/AgentToolsTab.tsx | variant 'review'\|'frontdesk' — Tools tab body: Connected tools (five already connected, mix of internal and external) plus an Add CTA. Add opens a portalled side panel (search, Internal tools / External tools tabs, All tools label, row cards; external rows have Connect). Review catalog matches the review-response tool list (Handle response, Select template, review tags, Salesforce/Freshdesk/QuickBooks/ServiceTitan/Zendesk). Connecting a tool adds it to the section. Used for review-response Tools tabs and, via `FrontdeskToolsTab`, Front desk (Sep 23) |
| GhostwriterKnowledgeTab | components/GhostwriterKnowledgeTab/GhostwriterKnowledgeTab.tsx | no props — the **Knowledge tab** of the same Ghostwriter create-flow shell (and of Front desk (Myna)'s canvas Knowledge tab). Header row (the "What this agent knows…" description on the left, an "Add knowledge" `rounded-sm` button on the right) above a bordered list of knowledge docs (icon, title, "Updated X ago" / source meta, right-aligned "Edit" text button). Hardcoded array in the file, not wired to a real knowledge store |
| GhostwriterRunTestModal | components/GhostwriterRunTestModal/GhostwriterRunTestModal.tsx | open, reviews (`Review[]` from `data/reviewsData.ts`), selectedIds (string[]), onToggleReview(id), onCancel, onRunTest — **Jay & Robin only**: the "Add test case" review picker opened from the Test tab. `createPortal`'d 1100px centered modal ("Add test case" + subtitle, ✕): left scrollable multi-select list — each row a squarish checkmark (`size-[18px] rounded-[2px]`, `border-control-border`/`bg-primary` — the same token-backed `Checkbox` look `FilesModal` uses, not a radio) plus a full `ReviewCardBody`, click-to-toggle — right a static "Filter by" column (Time period/Review source/Ratings/Review content — visual only, not wired); footer "N reviews selected" + Cancel / Run test (disabled until ≥1 picked). Portalled to `<body>` for the same reason `GhostwriterSimulation`'s panels are — the Ghostwriter shell's tab bar is a pinned `z-30` |
| GhostwriterTestRunPanel | components/GhostwriterTestRunPanel/GhostwriterTestRunPanel.tsx | batches (`TestRunBatch[]` — `{reviews: Review[], testedAt: string}`, one per confirmed "Run test", empty before the first), centerContent ((testRun: TestRunNodeState) => ReactNode — mounts the canvas, fed the same node-highlight state driving this panel's own stepper; ignored when `layout="fullpage"`), onRunTest?, className?, layout?='floating'\|'fullpage' — the whole Test tab, empty or filled, shared by **Jay & Robin and 23 Sep** (its straight duplicate). `layout="floating"` (default, Jay & Robin): LHS/RHS float (`FLOATING_PANEL_CLASS`, matching the Workflow tab's own floating chrome) over a full-bleed canvas that `centerContent` mounts. `layout="fullpage"` (23 Sep): the same LHS/RHS content instead fills a plain full-page `bg-surface` column (centered `max-w-[1200px]`, `border-r` between LHS/RHS, no floating chrome) with **no canvas at all** — same outer treatment as the Tools/Knowledge tabs. Empty: LHS titled "Test cases" (no "+ Add test case" button yet) with a centered icon + a caption ("Select a variety of reviews in your test case for best results.") + the "Add test case" CTA; RHS titled "Test" with a placeholder line. Filled: LHS header gains the "+ Add test case" button beside the title, then one group per batch — **newest first** (reversed for display only; the default "most recent" selection still reads the batches array in run order) — each its own "N review(s) tested" + timestamp row, reviews below (reviewer + `StarRating` + snippet + green check, click to select), `mt-lg` between groups; RHS ("Test" + `Chip variant="success"` "Passed") has Details/Preview tabs (`showBaseline={false}`) — Details reuses `TestRunPanel`'s exported `TestRunStepRow` + the `useTestRun` hook over `buildTestRunSteps(REVIEW_RESPONSE_WORKFLOW...)` (same stepper the canvas's own "Run test" uses), Preview shows the selected review's full `ReviewCardBody` with its existing simulated reply from `data/reviewsData.ts`. The floating layout's `ReviewWorkflowRun` and the full-page layout's `FullPageResultPanel` each own their own `useTestRun` call (keyed by review id so switching reviews restarts it) driving the shared, canvas-agnostic `TestResultBody` — only the floating one also feeds that state into `centerContent` |
| FrontdeskTestSessionsPanel | components/FrontdeskTestSessionsPanel/FrontdeskTestSessionsPanel.tsx | centerContent (ReactNode — caller mounts the canvas, ignored when `layout="fullpage"`), className?, layout?='floating'\|'fullpage' — the Test tab for **both Front desk (Myna) and Front desk (Sep 23)**, self-contained (owns its own session/batch/suite/personality state, seeded from `data/frontdeskTestSessions.ts`'s `FRONTDESK_TEST_BATCHES`). Also exports `SessionDetail` (the recording/transcript preview) so `FrontdeskTestRunReport` can reuse it verbatim. `layout="floating"` (default, Myna): same floating-panel language as `GhostwriterTestRunPanel`'s Jay & Robin mode — LHS "Tests" header + "Create test cases" CTA, Call/Web chat sub-`Tabs` filter the list, a contextual "Test call"/"Test webchat" button, then batches (newest first, tester name or "Myna" + sparkle + timestamp, each session a row). RHS: a row click shows `SessionDetail` (`CallRecordingPlayer`/`ChatBubble` transcript, read-only, `Passed`/`Failed` `Chip` beside the title, ✕ close); "Test call" mounts the real `PreviewPanel`; "Test webchat" mounts this file's own `TestWebChatPreview` ("Test Web Chat" title, composer prefills on focus, Send appends + one scripted reply) — neither commits back into the batch list in this mode. `layout="fullpage"` (Sep 23, mirrors `GhostwriterTestRunPanel`'s 23-Sep mode): no canvas, plain `bg-surface` column, a 200px `FrontdeskTestSectionNav` — **Tests runs / Test suite / Personality** (Test cycles hidden from the nav, same as the reference's own `TestSection`, kept only as a dead type). "Test runs" = `FrontdeskTestBatchSummaryCard`s fed by a single chevron-less `FrontdeskRunTestCtaButton` ("Create test") that opens `FrontdeskTestRunEditor` full-page (no dropdown) — a card with `runName` opens `FrontdeskTestRunReport` in place of the list on click, one without opens the older `FrontdeskTestBatchSessionsPanel` slide-in instead. `handleCreateRun` expands a picked test suite into one session per scenario (`buildBatchesFromDrafts`'s same per-scenario/channel logic, one batch this time not one-per-session) and tags `testedBy: 'Haresh'` — a named tester, so no sparkle icon — falling back to a single canned session when no suite was picked. "Test suite" = saved `FrontdeskTestSuite`s via `FrontdeskTestSuiteEditor`, defaulting to `data/frontdeskTestSessions.ts`'s `FRONTDESK_TEST_SUITES` (5 seeded suites named after common front-desk case categories — Billing issues, Unclear information, Appointment scheduling, Insurance verification, Escalations & complaints — each with its own themed `scenarios` array so the section and the Test Run editor's "Test suite" dropdown aren't empty by default); `handleSaveSuite` flips its `generating` flag off after 3s so a newly-created card settles on "{N} scenarios" instead of spinning forever. "Personality" renders `TestPersonalitySection`. Wired in `AgentDetailScreen.tsx` via `layout={isFrontdeskSep23Polish ? 'fullpage' : 'floating'}` inside the existing `isMynaCombinedNav` branch — Myna's own resolution is untouched |
| TestPersonalitySection | components/TestPersonalitySection/TestPersonalitySection.tsx | personalities (`TestPersonality[]`), onChange — Front desk (Sep 23) Test tab "Personality" section: a responsive card grid (name, fork/custom, enable switch, voice meta) plus a "Create personality" side panel (name, prompt, voice model/ID, interruption, speed, volume, noise, language, advanced accent/gender). State is owned by `FrontdeskTestSessionsPanel` so it survives a section switch |
| FrontdeskTestSuiteEditor | components/FrontdeskTestSuiteEditor/FrontdeskTestSuiteEditor.tsx | existingSuite?, onBack, onSave — Front desk (Sep 23) full-page create-suite flow: editable name, "Create N test scenarios using AI", upload-file link, then the scenario description textarea (pre-filled with a real, editable example — not a grey placeholder — for a brand-new suite). A "Generate scenarios" button sits directly below that textarea (outline style, distinct from the header's primary "Save test suite" CTA) — clicking it previews rows (cycling `FRONTDESK_SCENARIO_SUGGESTIONS` when the description was cleared, else repeating it) in a table below with **Call description / Expected outcome** columns (outcome cycled from a small canned-resolution pool), without saving. Opening an `existingSuite` that already has scenarios (the seeded `FRONTDESK_TEST_SUITES`, or any suite saved earlier) seeds this same preview table from `existingSuite.scenarios` up front, so it reads as already generated rather than needing "Generate scenarios" clicked again. "Save test suite" uses the previewed rows if any exist, else falls back to the old count-only generation — either way it sets `generating: true`, which `FrontdeskTestSessionsPanel.handleSaveSuite` flips off 3s later |
| FrontdeskTestRunEditor | components/FrontdeskTestRunEditor/FrontdeskTestRunEditor.tsx | defaultName, personalities, testSuites (`FrontdeskTestSuite[]`), onBack, onRun — Front desk (Sep 23) full-page create-run flow opened by the chevron-less "Create test" button. Editable name defaults to "#1 test run". The **first field is a single-select "Channel" dropdown** (Call/Webchat, defaults to Call — same dropdown shape as "Test suite" below it, just always has a value instead of a "Select…" placeholder), then a "Personas" multi-select, then a single-select "Test suite" dropdown (picks from `testSuites`, "No test suites yet." when empty), then the default plus response-quality evaluations (including latency). `onRun` receives `{ name, channel, personaIds, qualityEvaluationIds, suite }` (`channel: 'voice' | 'chat'`, `suite: FrontdeskTestSuite | null`) — `FrontdeskTestSessionsPanel.handleCreateRun` uses `channel` to filter which of a picked suite's scenarios (and which of the no-suite fallback session) actually run, instead of running every channel a scenario supports. Also exports `EvalTable`/`DEFAULT_EVALUATIONS`/`QUALITY_EVALUATIONS` so `GhostwriterTestRunEditor`/`FrontdeskTestRunReport` show the identical criteria instead of a copy |
| FrontdeskTestRunReport | components/FrontdeskTestRunReport/FrontdeskTestRunReport.tsx | batch (`FrontdeskTestBatch`), onBack, onAcceptRecommendation? — Front desk (Sep 23) Test tab's read-only page one level deeper than the Test runs list, opened by clicking a completed run (`batch.runName` set). Mirrors `GhostwriterTestRunReport` exactly but for calls/web chats instead of reviews: header (run name, "Run by {tester} · Test suite: {name} · {N} sessions tested · {timestamp}"), one stat tile per metric (`DEFAULT_EVALUATIONS` + whichever `QUALITY_EVALUATIONS` `batch.qualityEvaluationIds` turned on, both imported from `FrontdeskTestRunEditor`) averaged across `batch.sessions` via a deterministic `metricScore(sessionId, metricId)` mock, then Details/Recommendations tabs. Details = per-session score table (title + channel, a dedicated Status column right after Session with just the outcome icon — no "Passed"/"Failed" text — then one column per metric); clicking a row opens a right-anchored side panel (same `absolute right-0 top-0 h-full w-[560px]` shell `TranscriptSidePanel` uses, not a centered modal) reusing `FrontdeskTestSessionsPanel`'s exported `SessionDetail` verbatim. Recommendations = one card per failed session (no severity split — calls have no rating-style dimension to derive one from) with a generic fix suggestion + Accept, or the "no recommendations" empty line when every session passed |
| GhostwriterTestRunEditor | components/GhostwriterTestRunEditor/GhostwriterTestRunEditor.tsx | defaultName, testSuites (`TestSuite[]`), onBack, onRun — Response agent (23 Sep) Test tab's own full-page create-run flow, opened by `GhostwriterTestRunPanel`'s chevron-less "Run test" button (mirrors `FrontdeskTestRunEditor`'s pattern exactly, including its own internal `createRunOpen` state). Editable name defaults to "#1 test run"; below it a single-select "Test suite" dropdown (picks from `testSuites`, no picker modal) instead of Front desk's persona multi-select; then the same `EvalTable`-based Default/Response-quality evaluations imported from `FrontdeskTestRunEditor`. Submitting calls `onRun({ name, suite, qualityEvaluationIds })`; `GhostwriterTestRunPanel`'s `onRunTestWithSuite` prop turns that into a `TestRunBatch` with `runName` set (and `suiteName`/real pass-fail when a suite was picked) |
| GhostwriterTestRunReport | components/GhostwriterTestRunReport/GhostwriterTestRunReport.tsx | batch (`TestRunBatch`), onBack, onAcceptRecommendation? — read-only page one level deeper than the Test runs list, opened by clicking a completed run (any batch with `runName`, i.e. one created via `GhostwriterTestRunEditor`; older plain-picker batches have no evaluation metrics and keep opening `TestBatchReviewsPanel` instead). Header = run name, then `GhostwriterTestRunPanel`'s exported `testRunSubtitle(batch, total)` — "Run by {tester} · Test suite: {name} · {N} reviews tested · {timestamp}" (tester via its exported `pickTesterName`, a deterministic pick off the batch's own name+timestamp from the same Rupa/Akhil/Raynil/Haresh/John pool the agent directory cards use) — the exact same line `TestBatchSummaryCard`'s row shows, so the list and the detail page never disagree. No edit affordance anywhere — a run's name and results are fixed once it exists. Below that, one stat tile per metric (the always-on `DEFAULT_EVALUATIONS` plus whichever `QUALITY_EVALUATIONS` `batch.qualityEvaluationIds` turned on, both imported from `FrontdeskTestRunEditor`) showing that metric's average across `batch.reviews`; then Details/Recommendations tabs. Details = the per-review score table (review — reviewer/rating/snippet — as the first column, one column per metric; scores are a deterministic mock, `metricScore` hashed from `reviewId:metricId` so they don't reshuffle on reopen — percentages for every metric except `latency`, a real millisecond duration); clicking a row reopens `GhostwriterTestRunPanel`'s exported `TestReviewDetailModal` (the same Details/Preview pop-up `TestBatchReviewsPanel` used to open, `passed` via its exported `reviewPassedInBatch` — no per-review Recommendation tab, removed from that modal entirely since it duplicated the batch-level Recommendations tab below). Recommendations = the exact same theme-grouped fixes as `TestBatchReviewsPanel`'s own tab, reusing its exported `getBatchRecommendationGroups`/`RecommendationGroupCard` rather than re-deriving them |
...
| ReviewCardBody / StarRating | components/ReviewCard/ReviewCard.tsx | `ReviewCardBody`: review (`ReviewCardData`: reviewerName, rating, date, reviewId, location, text, reply?), replyFooter? (ReactNode rendered at the end of the reply block — `AllReviewsScreen` passes its thumbs-up/down + "Coach agent" / "Track your feedback" row for agent-written replies), className? — read-only review record (Birdeye avatar + `StarRating` + reviewer/date, reviewId/location tags, review text, and — when `reply` is set — the "Posted on {source logo} by {agent} • {date}" reply block). `StarRating`: rating, size?=18 — 5-star row (partial fill supported). Extracted from `AllReviewsScreen`'s `ReviewCard` (which wraps `ReviewCardBody` plus its own Reply/Edit-reply action buttons) so `RunDetailView`'s Review response "Review details" tab can render the same review layout for a log row |
...
| MessageTemplateModal | components/MessageTemplateModal/MessageTemplateModal.tsx | open, kind ('text'\|'email'), onClose, selected? (ids), onChange(ids), categories?, templates? — centered 1200px template library popup (Figma: "Text templates"): header with a per-kind icon (`mail` / `chat`) + "Text templates"/"Email templates" title, always-open search field and ✕; left 280px category rail (All + counts, blue left-bar active row, `chevron_right` per sub-category); right scrolling rows of thumbnail (mini CSS rendering of the message body) + title + 2-line preview. **Multi-select, no footer** — each row carries a checkbox and toggles immediately; ✕/Esc/outside-click just dismisses. Seeded from `data/messageTemplateLibrary.ts` (`getTemplateLibrary`/`getTemplateTitle`; category counts are the real library sizes, so they exceed the sampled rows). Opened by `DelayBody`'s Email/Text template fields (Delay → Event occurs). **Not** the same as `TemplatePicker/TemplatePicker.tsx`'s `TemplatePickerModal`, which is the multi-select reminder-template picker used by `SendReminderDrawer` |
...
| RecommendationsTab / RecommendationDetailScreen | screens/RecommendationsTab.tsx, screens/RecommendationDetailScreen.tsx | `RecommendationsTab`: agentName, onSelect(id), isDraft?, includeGenerated?=true (false = human coaching only, no `RECOMMENDATIONS`), extraItems? (seeded `Recommendation[]` shown until the store holds the same id), emptyTitle?/emptyDescription? — the **Coaching** table (columns Coaching / From / Date / Conversations affected / Status; the user's own submissions first, then seeds, then Myna's) for an agent's Coaching tab — `AgentInstanceScreen` — front desk shows Myna's + human items; a Jay & Robin / 23 Sep review response instance (`reviewCoaching` prop) shows the review coaching items instead (`REVIEW_COACHING_AGENT` + `REVIEW_COACHING_SEEDS`, after Logs, tab id still `'recommendation'`). Row click hands the id to the parent, which opens the workflow canvas with the copilot docked (`App.openCoachingCanvas`) instead of a full page. `RecommendationDetailScreen`: recommendationId, onBack, autoOpenFeedbackPrefill?, onAutoOpenFeedbackConsumed?, layout?='fullpage'\|'docked' — scripted AI chat walking through Issue/Impact/proposed change with an accept/reject flow; `docked` drops the page header and 720px max-width so the same content fits the 448px canvas copilot panel (`CoachingCanvasPane`). Ported from the `Akhil-myna-repo` branch's Recommendation flow (see `FeedbackRecommendationsStoreContext` / `RecommendationOverridesStoreContext` providers in `App.tsx`); the Inbox-to-Recommendation "human feedback" linkage from that branch (coaching examples, `submitFeedback` wiring) was intentionally left out of scope — our existing `agentFeedbackRecommendations.ts` + `InboxScreen` thumbs-down flow is untouched and separate |
...
### Look here first (hard rule)
Before any grep, Explore agent, or cold file read in `src/`, check `graphify-out/GRAPH_REPORT.md` (or run `graphify query`/`explain`/`path`) first, falling back to direct search only if the report doesn't resolve it — and after any change that adds, removes, or renames a symbol, run `graphify update .` then `graphify cluster-only . --no-label` before finishing.
