...
| VoiceCallEngineSettings | components/VoiceCallEngineSettings/VoiceCallEngineSettings.tsx | no props — STT/TTS engine model block (STT Model + Failover policy with a conditional fallback-Model picker shown only when policy='Manual'; TTS Model) used above Default voice in agent Settings and setup wizard Voice call settings |
| VoiceCallInterruptionSettings | components/VoiceCallEngineSettings/VoiceCallEngineSettings.tsx | no props — TTS Failover policy (same conditional fallback-Model picker) + Enable interruptions toggle; rendered after Default voice/Add additional voice and before Greeting message, sharing the file with `VoiceCallEngineSettings` |
