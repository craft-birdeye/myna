...
| BookingTemplateSelectField | components/BookingTemplateSelectField/BookingTemplateSelectField.tsx | label?, description?, value, onChange, templates ({id,name}[]), noneLabel?, onEditTemplate?(templateId) — dropdown of Booking template names from the live store + an "Edit template" link with a blue `open_in_new` icon that opens that template's editor. Shared by the Front desk agent Settings tab (agent inherits a template; default General intake) and `AppointmentWidgetEditorScreen`'s Configuration tab (widget is based on a template; default General intake, no "None") |
| BaseFieldsEditor | components/BaseFieldsEditor/BaseFieldsEditor.tsx | value (`{myself, someoneElse}` of `TemplateField[]`), onChange, readOnly? — "Myself / Someone else" tabbed base-fields list (type icon, label, required *, remove for non-system fields, "+ Add field to base form"); `readOnly` grays rows and hides add/remove (used when a widget inherits fields from a template). Also exports `AddFieldForm` (Label + Field type + Required mini-form, shared with a template's field-group "extra fields"), `FieldTypeIcon`, `fieldTypeLabel`, `SimpleSelect` |
| BookingTemplatesScreen / BookingTemplateEditorScreen | screens/BookingTemplatesScreen.tsx, screens/BookingTemplateEditorScreen.tsx | Front desk → Resources → "Booking templates" (New badge, healthcare + dental only — automotive has no per-provider booking concept). List = table (Template name purple chip, Services "N selected"/"All services", Field groups count, Used by) + "Create template" CTA + explainer banner. Editor = back+name(pencil rename)+[Template] chip header with Cancel/Save (new) or ⋮ Actions(Delete)/Save (existing); three bordered sections: Base form fields (`BaseFieldsEditor`), Appointment types and providers (two multi-selects, empty = all), Field groups (table + modal). `initialEditId` opens a template's editor directly (from agent Settings / widget Configuration "Edit template"). Data model + seed templates ("General intake", "Surgical template") in `data/bookingTemplatesData.ts`, session store in `data/BookingTemplateStoreContext.tsx` |
| AppointmentWidgetEditorScreen | screens/AppointmentWidgetEditorScreen.tsx | widget ({name,location,createdBy,createdOn}), onBack, onOpenBookingTemplates?(templateId) — Settings → Widgets → Appointment widgets → row click/Edit opens this. Install/Appearance/Configuration/Preferences tabs (only Configuration has real content). Configuration = "Widget-only settings" info card → "Widget title and description" → "Based on template" (`BookingTemplateSelectField`, default General intake, no None) → Base form fields (read-only inherited `BaseFieldsEditor` from the selected template) |
...
- **Booking templates:** new Front desk → Resources item (healthcare + dental only, right below Availability) — see the `BookingTemplateSelectField` / `BaseFieldsEditor` / `BookingTemplatesScreen` / `AppointmentWidgetEditorScreen` registry rows above. A template is the shared, reusable form-fields/services/providers unit: a Front desk agent inherits one (Settings tab "Booking template" card) and an appointment widget optionally starts from one (Configuration tab "Based on template") — neither is "wired to" the other, they just both read the same `BookingTemplateStoreContext`. `onOpenBookingTemplates` is threaded `App.tsx` → `AgentDetailScreen` → `AgentInstanceScreen` → `AgentSettingsTab` (and separately `App.tsx` → `AppointmentWidgetsScreen` → `AppointmentWidgetEditorScreen`) for the "Edit template" links to jump straight to the Booking templates list.
