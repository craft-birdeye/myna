...
| GhostwriterTestRunPanel | components/GhostwriterTestRunPanel/GhostwriterTestRunPanel.tsx | batches (`TestRunBatch[]` — `{reviews: Review[], testedAt: string}`, one per confirmed "Run test", empty before the first), centerContent ((testRun: TestRunNodeState) => ReactNode — caller mounts the canvas, fed the same node-highlight state driving this panel's own stepper), onRunTest?, className? — **Jay & Robin only**: the whole Test tab, empty or filled. Floating LHS/RHS cards (`FLOATING_PANEL_CLASS`, matching the Workflow tab's own floating chrome) over a full-bleed canvas, not flex columns. Empty: LHS titled "Test cases" (no "+ Add test case" button yet) with a centered "Add test case" CTA + a caption ("Select a variety of reviews in your test case for best results."); RHS titled "Test" with a placeholder line. Filled: LHS header gains the "+ Add test case" button beside the title, then one group per batch — **newest first** (reversed for display only; the default "most recent" selection still reads the batches array in run order) — each its own "N review(s) tested" + timestamp row, reviews below (reviewer + `StarRating` + snippet + green check, click to select), `mt-lg` between groups; RHS ("Test" + `Chip variant="success"` "Passed") has Details/Preview tabs (`showBaseline={false}`) — Details reuses `TestRunPanel`'s exported `TestRunStepRow` + the `useTestRun` hook over `buildTestRunSteps(REVIEW_RESPONSE_WORKFLOW...)` (same stepper the canvas's own "Run test" uses; `ReviewWorkflowRun` is keyed by review id so switching reviews restarts it), Preview shows the selected review's full `ReviewCardBody` with its existing simulated reply from `data/reviewsData.ts` |
| FrontdeskTestSessionsPanel | components/FrontdeskTestSessionsPanel/FrontdeskTestSessionsPanel.tsx | centerContent (ReactNode — caller mounts the canvas), className? — **Front desk (Myna) only**: the Test tab (ordered right after Workflow), same floating-panel language as `GhostwriterTestRunPanel` but self-contained (owns its own session/batch state, seeded from `data/frontdeskTestSessions.ts`'s `FRONTDESK_TEST_BATCHES`). LHS "Tests" header + a "Generate testcases" CTA (two-star `.ai-flat-sparkle-icon`, matching `currentColor`); Call/Web chat sub-`Tabs` filter the list; below them a contextual "Test call"/"Test webchat" button; then batches — newest first, header = tester name (or "Myna" + violet sparkle for AI-generated ones) + timestamp — each session a row (outcome icon, title, first transcript line). RHS has three states: picking a row shows a static "Preview" (recording via `CallRecordingPlayer` for voice + `ChatBubble`/`ChatSystemLabel` transcript, read-only); "Test call" mounts the real `PreviewPanel` (same live call/chat simulator Front desk agent (Sep 1) shows); "Test webchat" mounts this file's own `TestWebChatPreview` (titled "Test Web Chat" — `PreviewPanel`'s chrome is fixed and can't carry a different title — greeting bubble, composer prefills a canned message on focus, Send appends it + one scripted reply). "Generate testcases" reveals `FRONTDESK_GENERATED_TEST_POOL` one batch at a time (900ms apart) via a local `generating`/`generateStep` effect. The canvas's own top-bar "Run test" is hidden for this nav (`hideRunTest`) since testing lives here instead |
