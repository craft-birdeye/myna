...
| ProcedureSidePanel | components/ProcedureSidePanel/ProcedureSidePanel.tsx | open, title, whenToUse?, steps (`{title, bullets}[]`), exitCriteria?, onClose — read-only slide-in panel showing a procedure's full when-to-use + numbered steps + when-to-exit; opened from a "Procedure updated/created" block's "View Procedure" button in the recommendation chat. Bullet text supports inline `{{token}}` markup, rendered as read-only `RefChip` (kind='tool') pills |
