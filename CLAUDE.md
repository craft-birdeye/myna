...
| MessageTemplateModal | components/MessageTemplateModal/MessageTemplateModal.tsx | open, kind ('text'\|'email'), onClose, onSelect(template), categories?, templates? — centered 1200px template library popup (Figma: "Text templates"): header with a per-kind icon (`mail` / `chat`) + "Text templates"/"Email templates" title, always-open search field and ✕; left 280px category rail (All + counts, blue left-bar active row, `chevron_right` per sub-category); right scrolling rows of thumbnail (mini CSS rendering of the message body) + title + 2-line preview. **Single-select, no footer** — clicking a row commits and closes. Seeded from `data/messageTemplateLibrary.ts` (`getTemplateLibrary`/`getTemplateTitle`; category counts are the real library sizes, so they exceed the sampled rows). Opened by `DelayBody`'s Email/Text template fields (Delay → Event occurs). **Not** the same as `TemplatePicker/TemplatePicker.tsx`'s `TemplatePickerModal`, which is the multi-select reminder-template picker used by `SendReminderDrawer` |
