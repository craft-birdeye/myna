...
|-----------|------|---------------|
| CopilotPanel | components/CopilotPanel/CopilotPanel.tsx | agentName, userName?='John', onClose, onExpand?, onOpenRecommendation? — THE docked Copilot surface (400px, expand→640px) that squeezes page content; replaces the old AiAssistPanel/CoachAgentPanel. Home view = greeting + centered BirdAI sparkle + suggestion chips + composer; a `history` icon in the header opens the per-agent "Conversations" list view (from `useCopilotThreadsStore`); sending opens an `ask` thread with a scripted reply. Threads with a `recommendationId` (coaching feedback, recommendation visits) call `onOpenRecommendation` instead of opening in-panel. Mounted on `AgentInstanceScreen` (sparkle button, non-recommendation tabs), the workflow editor (App.tsx + AgentBuilder), and the create-flow split view |
...
| LogDetailsPanel | components/LogDetailsPanel/LogDetailsPanel.tsx | row (HealthcareLogRow), agentName?, callerNumber?, sidNumber?, languageDetected?, callEndReason?, routedVia?, summary?, transcript?, durationSecs?, audioUrl?, onViewConversation? — conversation details side panel (meta card, collapsible AI summary, call recording via `CallRecordingPlayer`, call transcript). Agent transcript bubbles show thumbs up/down (same `ShareFeedbackModal` + `useFeedbackRecommendationsStore().submitFeedback` flow as `InboxScreen`), tagged with `agentName` so it lands on that instance's Recommendation tab |
