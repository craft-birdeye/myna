...
| SuperAgentApp | components/SuperAgentApp/SuperAgentApp.tsx | active, title?, enter? (`SuperAgentEnterCommand`: ts, isNewUser?), onBackToBirdeye? — full-viewport `fixed inset-0 z-50` overlay (not embedded in the dashboard's own L1/L2 — the iframe draws its own chrome entirely: its own left nav, its own "Super Agent  BIRDEYE ⌄" `BirdeyeSwitcher`) embedding the self-contained Super Agent prototype from `public/super-agent-prototype.html` with no `chrome`/`stage` query params. `enter.isNewUser` picks which of the prototype's own two entry functions to mirror: `true` (Website gate's "new user") posts `{ type: 'superagent:enter-new' }`, mirroring `replayOnboarding()` — a genuine sign-up from scratch: auth (sign-up) then the *full* onboarding question flow, resetting any prior setup; `false`/omitted (TopBar `AppSwitcher`, L1 rail item) posts `{ type: 'superagent:enter' }`, mirroring `openSuper()` — skip straight to a *short* onboarding the first time, or straight to the workspace on every visit after (decided by the iframe's own `setup.onboarded`). Both message types are queued if the iframe hasn't fired its `load` event yet (a message posted before the iframe's script has run and attached its listener is silently dropped) and flushed once it has. `onBackToBirdeye` fires when the prototype's own `BirdeyeSwitcher` "Birdeye / Back to your dashboard" row is clicked — its `app.backToBirdeye()` now also does `window.parent.postMessage({ type: 'superagent:back-to-birdeye' }, '*')` (in addition to its own internal `setStage("dash")`, harmless when run standalone) so the host can close the overlay and reveal the real Dashboard underneath. Kept mounted (hidden via CSS, not unmounted) once opened, because the prototype compiles its own JSX on load and a remount costs a 1–3s blank frame. `App.tsx` also hides its entire dashboard shell (`className="hidden"` on the outermost flex div, not just z-index) while the overlay is open, so the L1 rail/TopBar can never bleed through underneath it |
