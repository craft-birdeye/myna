...
| SuperAgentApp | components/SuperAgentApp/SuperAgentApp.tsx | active, title?, mode?='overlay'\|'embedded', enter? (`SuperAgentEnterCommand`: ts, isNewUser?), navigate? (`SuperAgentNavigateCommand`: key, ts), onBackToBirdeye? — `mode='overlay'` (default, used by the Website gate + TopBar `AppSwitcher`): full-viewport `fixed inset-0 z-50`, iframe draws its own chrome entirely (its own left nav, its own "Super Agent  BIRDEYE ⌄" `BirdeyeSwitcher`), no `chrome` query param. `mode='embedded'` (used **only** by the L1 rail item's "Create agent" row — see below) : `flex-1 min-h-0` sized to fill its container below a sticky host header, `?chrome=none` hides the iframe's own left nav since the L2 SideNav already provides navigation. `enter.isNewUser` picks the prototype's own entry function to mirror (`true` -> `superagent:enter-new`/`replayOnboarding()`, full sign-up; `false`/omitted -> `superagent:enter`/`openSuper()`, short onboarding once then straight to the workspace) — only ever sent in `overlay` mode. `navigate` posts `{ type: 'superagent:navigate', key }`, which the iframe's handler treats as "skip onboarding, force `stage: 'app'`, jump to this nav key" — this is the **only** message the embedded L1 module ever sends, so it never shows onboarding/sign-up. Both message types queue if the iframe hasn't fired `load` yet and flush once it has. `onBackToBirdeye` fires on the prototype's own "Back to Birdeye" click (overlay mode only). Kept mounted (hidden via CSS) once opened — the prototype compiles its own JSX on load and a remount costs a 1–3s blank frame. `openAgentCmd`/`useLibraryCmd` (embedded only) post `superagent:open-agent`/`superagent:use-library-agent`, which the prototype's message handler turns into `app.openAgent(id)`/`app.useLibAgent(key)` — opening its full-page `AgentScreen`. `onCloseAgent` fires when that `AgentScreen`'s back chevron (rendered only when `EMBED_NO_CHROME`) posts `superagent:close-agent`. The prototype's own `AgentAvatarImg` also checks `EMBED_NO_CHROME` and always falls back to its flat neutral Sparkle chip instead of each agent's colorful illustration when embedded, so Create agent/My agents/Library read as one consistent icon language — the standalone overlay keeps its original colorful set |
| SuperAgentMyAgentsScreen / SuperAgentLibraryScreen / SuperAgentKnowledgeScreen / SuperAgentConnectionsScreen | screens/superAgent/{SuperAgentMyAgentsScreen,SuperAgentLibraryScreen,SuperAgentKnowledgeScreen,SuperAgentConnectionsScreen}.tsx | `SuperAgentMyAgentsScreen`: onOpenAgent(id). `SuperAgentLibraryScreen`: onUseAgent(key). `SuperAgentKnowledgeScreen`: no props — a **flat hub of every knowledge source across all agents** (not grouped by agent): flattens every `SuperAgentMyAgent.knowledge` item (`knowledge: SuperAgentKnowledgeItem[]` field in `superAgentSeedData.ts`, kind `'file'|'image'|'link'|'note'`) across `SUPER_AGENT_ACTIVE_AGENTS`+`SUPER_AGENT_PAUSED_AGENTS` into one `DataTable` (Name w/ kind icon, Type `Chip`, Source agent, Updated — all `sortable`, using DataTable's own built-in column-header sort). Header has `HeaderSearchField` (matches title/source/detail), an "Add files" button opening `FilesModal` to append picked files as new rows tagged `source: 'You'`, and a filter-icon button toggling a `FilterPanel` (Type + Source agent fields, wired via `selections`/`onSelectionsChange`) as a side panel next to the table. Row three-dot menu only offers "Remove", and only for user-added rows (`rowMenuItems[].visible`). Mirrors the prototype iframe's own new top-level "Knowledge" nav row (`public/super-agent-prototype.html`'s `NAV` array + `KnowledgeScreen` component — same flat-hub idea: `flattenKnowledge(app.agents)` + local `uploaded` state for "Add knowledge", plus its own search input, a type-filter dropdown, and a "Sort: Name/Source agent/Type" dropdown, since the iframe has no shared `DataTable`/`FilterPanel` to reuse — kept in sync by hand since the iframe is a separate app). `SuperAgentConnectionsScreen`: no props — the Super agent L1 module's "My agents"/"Library"/"Knowledge"/"Connections" L2 rows (`SUPER_AGENT_NAV_SECTIONS` in `App.tsx`, all native now except "Create agent"). Native myna screens (not the prototype iframe): sticky `bg-surface px-2xl py-xl` header (no tab row — My agents dropped its Activity/Reports tabs, it's a single list now), full-width `bg-white` body (no `mx-auto`/`max-w` cap) with CLAUDE.md §6.7 primary/secondary buttons, `MetricTiles`, `Chip`, and `InfoCard` (Library grid, all 19 of the prototype's own `LIB_AGENTS` ported into `superAgentSeedData.ts` — not a hand-picked subset). Connections' "Reach your agents from another app" cards use `ChannelGlyph.tsx` (real inline-SVG brand marks for WhatsApp/Slack/iMessage/Telegram, ported from the prototype's own `ChannelGlyph`) and each has a "Connect"/"Manage" button opening `ChannelConnectModal.tsx` (ported from the prototype's own `ChannelConnectModal`: QR + pairing phrase + a glimpse of the thread, data from `SUPER_AGENT_CHANNEL_SETUP` in `superAgentSeedData.ts`). Its primary CTA ("Open WhatsApp"/etc.) connects and opens `ChannelPreviewModal.tsx` — a phone-frame mock of the actual thread ("the mobile preview screen"); "See how it reads" opens the same preview without connecting. The "Data sources" section is `ConnectorBrowser.tsx`: a category rail + search over `superAgentConnectorCatalog.ts`'s real, accurately-branded connector catalog (Communication/Reviews & listings/Social/Other) instead of a flat list — real logo assets come from `superAgentConnectionLogos.ts` (Google/Facebook/Instagram/LinkedIn/Twitter/YouTube/Teams/Outlook/Yelp/Birdeye) or `ChannelGlyph` for the 4 messaging brands; only Birdeye's own capabilities (appointment calendar, phone number, website chat) fall back to a plain Material icon, since there is no outside logo to be accurate to. Connecting one of the 4 messaging brands from inside the browser opens the same `ChannelConnectModal`. **Opening an agent:** "Open agent"/"Use agent" don't navigate anywhere themselves — they call `onOpenAgent(id)`/`onUseAgent(key)`, which `App.tsx` turns into a `SuperAgentApp` `openAgentCmd`/`useLibraryCmd` (posts `superagent:open-agent`/`superagent:use-library-agent` into the iframe, which calls the prototype's own `app.openAgent(id)`/`app.useLibAgent(key)`) and flips `superAgentViewingAgent` true, which swaps this L2 row's native screen out for the iframe (now showing the prototype's own full-page `AgentScreen` — Chat/Workflow/Approvals/Activity/Reports/Knowledge/Connections/Settings tabs). `AgentScreen` only renders a back chevron when `EMBED_NO_CHROME` (this module) — clicking it posts `superagent:close-agent` back to the host, which `App.tsx` turns into `superAgentViewingAgent = false`, revealing the native screen again. `id`/`key` values in `superAgentSeedData.ts` are exactly the prototype's own agent ids / library keys, so the handoff always resolves. Only "Create agent" (the 4th L2 row) still opens the prototype iframe directly — it's the AI-builder chat experience, not replicable natively |
| AppSwitcher | components/AppSwitcher/AppSwitcher.tsx | onSuperAgent, onSelectBirdeye, onSelectSuperAgent — global TopBar dropdown (replaces the old plain `moduleTitle` text), restyled to match the *shape* of the prototype's own two switchers (icon + title + small-caps tag + chevron trigger; a promoted icon+title/subtitle+chevron row to the other app; a divider; a quiet "You are on X · Current" row) — but always regular font weight per CLAUDE.md §6.6, never the prototype's `font-semibold`. Offers a second way into the Super agent overlay besides the Website gate and L1 rail item. `onSelectSuperAgent` is wired to `App.tsx`'s `openSuperAgent()` (default `isNewUser=false`, i.e. the `openSuper()`-mirroring quick-entry path); `onSuperAgent` is always passed `false` since the TopBar (and this dropdown) are only ever visible while on the Birdeye dashboard — the overlay covers the whole viewport including the TopBar when open |
| WebsiteGateScreen | screens/WebsiteGateScreen.tsx | onCurrentUser, onNewUser — full-screen front gate shown before Overview on every fresh load (`App.tsx`'s `hasEnteredApp`, always starts `false`). Renders `public/website-gate.svg` (the literal, unmodified Figma export of the real Birdeye marketing site — every glyph is a path, so there's no live text to hook into) as a full-bleed background sized via an `aspectRatio: '1920 / 1067'` wrapper so it never crops or distorts, with a single invisible hotspot button positioned by percentage (`DASHBOARD_HOTSPOT`, measured directly off the SVG's own viewBox) over its "Dashboard" pill. Clicking it opens a "Continue as" menu (Current user / New user, matching the design the Super agent prototype itself uses for this choice). "Current user" flips `hasEnteredApp` true, landing on the default Overview — the real Birdeye Dashboard, nothing else changes. "New user" additionally calls `App.tsx`'s `openSuperAgent(true)`, which opens `SuperAgentApp`'s full-page overlay on top of the (also-mounting) Dashboard with `isNewUser=true` — landing a new user in the prototype's full sign-up + onboarding flow, never its website/dashboard mockup |
