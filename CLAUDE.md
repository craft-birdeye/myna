...
| AudioPreviewPlayer | components/AudioPreviewPlayer/AudioPreviewPlayer.tsx | audioUrl?, durationSecs?, caption? — simplified greeting preview player (WaveSurfer waveform + play/pause + elapsed/total time; no speed control). Used on `FallbackFailoverScreen` |
...
| BookingTemplateSelectField | components/BookingTemplateSelectField/BookingTemplateSelectField.tsx | label?, description?, value, onChange, templates ({id,name}[]), noneLabel?, onEditTemplate?(templateId) — dropdown of Booking template names from the live store + an "Edit template" link that opens that template's editor. Shared by the Front desk agent Settings tab (agent inherits a template; default General intake, card sits right after System prompt) and `AppointmentWidgetEditorScreen`'s Configuration tab (widget is based on a template; default General intake, no "None") |
| BaseFieldsEditor | components/BaseFieldsEditor/BaseFieldsEditor.tsx | value (`{myself, someoneElse}` of `TemplateField[]`), onChange, readOnly? — "Myself / Someone else" tabbed base-fields list (type icon, label, required *, inline edit + remove for non-system fields, "+ Add field"); choice-type fields (dropdown/multiple-choice) show their answer options via `MoreLabelsCell`. Custom (non-system) fields get a `drag_indicator` handle next to the remove icon — native HTML5 drag-and-drop, live-reorders on `dragover` (same pattern as `CustomizeColumnsDrawer`), dimming the dragged row (`opacity-40`) with a `transition-[opacity,background-color]` for a smooth feel; since system fields are always the fixed leading block, a dragged custom field can never be dropped above the first non-system index — it can only reorder within the custom-fields tail. `readOnly` grays rows and hides add/edit/remove/drag (used by `AppointmentWidgetEditorScreen`'s read-only "Form fields" summary once a template drives the widget). Also exports `AddFieldForm` (Label + Field type [icon-labeled `SimpleSelect`] + answer-options editor for choice types + Required, doubles as an edit form when passed a `field` prop), `FieldTypeIcon`, `fieldTypeLabel`, `SimpleSelect` |
| MoreLabelsCell | components/MoreLabelsCell/MoreLabelsCell.tsx | labels (string[]), emptyLabel?, maxVisible?=1, className? — "Label, +N more" with hover-to-reveal popover (portaled + fixed-positioned so table-cell truncation can't clip it). Plain-text style (no chips, no underline), black text throughout. Shared by the Booking templates list (Appointment types column), the template editor's Field groups table (Mapped appointment types / Extra fields columns), and `BaseFieldsEditor`'s choice-field option summaries |
| BookingTemplatesScreen / BookingTemplateEditorScreen | screens/BookingTemplatesScreen.tsx, screens/BookingTemplateEditorScreen.tsx | Front desk → Resources → "Booking templates" (healthcare + dental only). List = plain `DataTable` (Template name, Appointment types via `MoreLabelsCell`, Field groups count, Used by all locked-visible; Created by/Created date/Updated by/Last updated available via Customize columns) + search + "Create template" CTA + Customize-columns + Filters (Appointment types / Field groups / Used by, all real). No banner on the list — instead, saving an existing template that's `usedBy` an Agent/Widget shows a confirm-style "This template is in use" popup ("Saving will update it everywhere it's used") before committing, and stamps `updatedBy`/`updatedDate`. Editor = back+name(pencil rename) header with Cancel/Save (new) or ⋮ Actions(Delete)/Save (existing); sections: Base form fields (`BaseFieldsEditor`), Appointment types and providers (two multi-selects, empty = all), Field groups (`DataTable` with `MoreLabelsCell` cells + hover pencil `rowAction`, no chips). "Add field group" opens a right-side **drawer** (`FieldGroupDrawer`, always-mounted + `translate-x` transition like `CustomizeColumnsDrawer`, not a centered modal) — Group name / Appointment types / Extra fields headings share one consistent label style, no dividers between header/body/footer; the Appointment types checklist is flat (no bordered box, just stacked `CheckboxRow`s); Extra fields rows get the same `drag_indicator` handle next to the close icon as `BaseFieldsEditor` (all extra fields are freely reorderable, no locked/system segment here). `initialEditId` opens a template's editor directly (from agent Settings / widget Configuration "Edit template"). Data model (incl. `createdBy/createdDate/updatedBy/updatedDate`) + 7 seed templates in `data/bookingTemplatesData.ts` (appointment types sourced from `data/appointmentTypesData.ts`), session store in `data/BookingTemplateStoreContext.tsx` |
| AppointmentWidgetEditorScreen | screens/AppointmentWidgetEditorScreen.tsx | widget ({name,location,createdBy,createdOn}), onBack, onOpenBookingTemplates?(templateId) — Settings → Widgets → Appointment widgets → row click/Edit opens this. Breadcrumb (Settings/Widgets/Appointments/name) + header (title+pencil, location link, gear/Cancel/Save) + Install/Appearance/Configuration/Preferences tabs (only Configuration has real content), matching the real product's true half/half two-column layout (`lg:grid-cols-2`): LEFT is one continuous divided card — Widget title and description → Booking template (`BookingTemplateSelectField`, no inline field builder — form fields live entirely in the template) → "Allow customers to choose service providers at unintegrated locations" toggle → Booking preferences (inline "show slots up to N months" / "minimum notice N day(s)" selects); RIGHT is a full-height `bg-surface-l2` pane with a sticky live Preview card matching the real widget chrome — brand header (circular icon + location, Call/Email pills), left sub-column with the typed booking header/description + Select location/Appointment type/Provider selects, right sub-column with a static Aug-2026 calendar + 2-column time-slot grid, Next button, and a "Powered by [icon] Birdeye" footer (desktop/mobile device toggle collapses the two sub-columns to one) |
...
- **Booking templates:** Front desk → Resources item (healthcare + dental only, right below Availability) — see the `BookingTemplateSelectField` / `BaseFieldsEditor` / `MoreLabelsCell` / `BookingTemplatesScreen` / `AppointmentWidgetEditorScreen` registry rows above. A template is the shared, reusable form-fields/appointment-types/providers unit: a Front desk agent inherits one (Settings tab "Booking template" card, placed right after System prompt) and an appointment widget optionally starts from one (Configuration tab "Booking template") — neither is "wired to" the other, they just both read the same `BookingTemplateStoreContext`. `onOpenBookingTemplates` is threaded `App.tsx` → `AgentDetailScreen` → `AgentInstanceScreen` → `AgentSettingsTab` (and separately `App.tsx` → `AppointmentWidgetsScreen` → `AppointmentWidgetEditorScreen`) for the "Edit template" links to jump straight to that template's editor (`initialEditId`). No banner on the list — impact is instead surfaced as a "This template is in use" confirm popup on Save (only when `usedBy` is non-empty), which also stamps `updatedBy`/`updatedDate`. "Add field group" is a right-side drawer (not a centered modal) for a smooth slide-in/out.
