...
| FrontdeskTestSessionsPanel | components/FrontdeskTestSessionsPanel/FrontdeskTestSessionsPanel.tsx | centerContent (ReactNode — caller mounts the canvas, ignored when `layout="fullpage"`), className?, layout?='floating'\|'fullpage' — the Test tab for **both Front desk (Myna) and Front desk (Sep 23)**, self-contained (owns its own session/batch/suite state, seeded from `data/frontdeskTestSessions.ts`'s `FRONTDESK_TEST_BATCHES`). `layout="floating"` (default, Myna): same floating-panel language as `GhostwriterTestRunPanel`'s Jay & Robin mode — LHS "Tests" header + "Create test cases" CTA, Call/Web chat sub-`Tabs` filter the list, a contextual "Test call"/"Test webchat" button, then batches (newest first, tester name or "Myna" + sparkle + timestamp, each session a row). RHS: a row click shows a static "Preview" (`CallRecordingPlayer`/`ChatBubble` transcript, read-only); "Test call" mounts the real `PreviewPanel`; "Test webchat" mounts this file's own `TestWebChatPreview` ("Test Web Chat" title, composer prefills on focus, Send appends + one scripted reply) — neither commits back into the batch list in this mode. `layout="fullpage"` (Sep 23, mirrors `GhostwriterTestRunPanel`'s 23-Sep mode exactly): no canvas, plain `bg-surface` centered `max-w-[1200px]` column, a 200px `FrontdeskTestSectionNav` (Tests/Test suite/Test cycles, science/fact_check/autorenew icons) driving the main pane. "Tests" = `FrontdeskTestBatchSummaryCard`s (pass-count `Chip` + timestamp) in place of the floating list, fed by a `FrontdeskRunTestCtaButton` "Run test ⌄" dropdown with **three** options — "Test call"/"Test webchat" (open the *same* right-side panel as Myna's, now `fixed`-positioned since there's no canvas to float over; ending the call/closing the webchat — `onSessionEnded`/`TestWebChatPreview`'s new `onEnd` — commits a new card) and "Use test suite" (disabled until ≥1 suite saved). "Test suite" = saved `FrontdeskTestSuite`s (name + scenario count); its empty/"Create test suite" CTA reuses `CreateTestCasesModal` verbatim via new `title`/`submitLabel` props, saving instead of running. "Test cycles" = a plain empty state, no CTA (matches the review-response reference's own unbuilt placeholder). A card click opens `FrontdeskTestBatchSessionsPanel` (right slide-in listing that batch's sessions), and picking a session there shows it on the same RHS panel via the existing `SessionDetail`. Wired in `AgentDetailScreen.tsx` via `layout={isFrontdeskSep23Polish ? 'fullpage' : 'floating'}` inside the existing `isMynaCombinedNav` branch — Myna's own resolution is untouched. The canvas's own top-bar "Run test" stays hidden for both navs (`hideRunTest`) since testing lives here instead |
...
| FrontdeskTestSuiteEditor | components/FrontdeskTestSuiteEditor/FrontdeskTestSuiteEditor.tsx | existingSuite?, onBack, onSave — Front desk (Sep 23) full-page create-suite flow: editable name, "Create N test scenarios using AI", upload-file link, scenario description. Header CTA is "Generate scenarios"; saving sets `generating` so the suite list shows a spinner and "In progress" in place of the scenario count |
| FrontdeskTestRunEditor | components/FrontdeskTestRunEditor/FrontdeskTestRunEditor.tsx | defaultName, personalities, onBack, onRun — Front desk (Sep 23) full-page create-run flow opened by the chevron-less "Run test" button. Editable name defaults to "#1 test run". Holds the persona multi-select and default plus response-quality evaluations (including latency) that used to live on the suite editor. Also exports `EvalTable`/`DEFAULT_EVALUATIONS`/`QUALITY_EVALUATIONS` so `GhostwriterTestRunEditor` shows the identical criteria instead of a copy |
