...
| PromptComposer | components/PromptComposer/PromptComposer.tsx | value, onChange, onSend, placeholder?, disabled?, sendDisabled?, rows?=2, attachments? (`{id,kind,label}[]`), onRemoveAttachment?, onAttach? (renders `AttachMenuPopover` when set, else a plain non-interactive "+"), onFocus?, onClick?, className? — canonical chat composer (rounded-xl card, `RefChip` attachment row, textarea, attach + dictate + `SendIcon` send button). Shared by `AgentDetailScreen`'s create-flow follow-up composer and `LHSDrawer`'s saved-transcript replay (replaces the old workflow-only `AIPromptBox`) so both surfaces render the same input |
| CreateAgentLanding | components/CreateAgentLanding/CreateAgentLanding.tsx | greeting, options (`{label,onSelect}[]`), value, onChange, onSend, placeholder?, disabled?, attachments? (`{id,kind,label}[]`), onRemoveAttachment?, onAttach? (renders `AttachMenuPopover` when set), className? — the canonical "Create with AI" welcome/empty state: 40px `AiAvatarChatIcon` + greeting + vertically-stacked full-width quick-start buttons, plus its own larger composer (`min-h-40`, attach + dictate + "Add context" + "More options" + send). Shared by `AgentDetailScreen`'s create-flow landing and `LHSDrawer`'s "Create with AI" tab (before any transcript exists) so both surfaces are one component, not two — update this file, not each call site, when this state needs to change |
