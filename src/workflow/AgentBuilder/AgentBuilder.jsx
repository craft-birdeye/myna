...
export default function AgentBuilder({
  ...
  // Create-with-AI composer: when focused, canvas node clicks attach as pills instead of opening RHS.
  const [aiComposerAttachments, setAiComposerAttachments] = useState([]);
  const aiComposerArmedRef = useRef(false);
  const aiComposerBlurTimerRef = useRef(null);

  const armAiComposer = useCallback(() => {
    if (aiComposerBlurTimerRef.current) {
      clearTimeout(aiComposerBlurTimerRef.current);
      aiComposerBlurTimerRef.current = null;
    }
    aiComposerArmedRef.current = true;
  }, []);

  const disarmAiComposerSoon = useCallback(() => {
    if (aiComposerBlurTimerRef.current) clearTimeout(aiComposerBlurTimerRef.current);
    aiComposerBlurTimerRef.current = setTimeout(() => {
      aiComposerArmedRef.current = false;
      aiComposerBlurTimerRef.current = null;
    }, 250);
  }, []);

  const handleRemoveComposerAttachment = useCallback((id) => {
    setAiComposerAttachments((prev) => prev.filter((a) => a.id !== id));
  }, []);

  const handleClearComposerAttachments = useCallback(() => {
    setAiComposerAttachments([]);
  }, []);
  ...
  const handleNodeClick = useCallback((node) => {
    ...
    // Create-with-AI composer is focused → also attach the node as a pill (RHS still opens).
    if (aiComposerArmedRef.current) {
      ...
      const label =
        ...
        details.agentName ||
        node.data?.title ||
        node.data?.headerLabel ||
        (node.type === 'start' ? agentName || 'Start' : null) ||
        'Node';
      // Chip chrome; swatch SVG comes from canvasNodeType (TaskIcon, TriggerIcon, etc.).
      const kind =
        node.type === 'trigger' || node.type === 'start' ? 'tool' : 'procedure';
      const canvasNodeType = node.type || 'task';
      const attachmentId = `canvas-${node.id}`;
      setAiComposerAttachments((prev) => {
        if (prev.some((a) => a.id === attachmentId)) return prev;
        return [...prev, { id: attachmentId, kind, label, canvasNodeType }];
      });
    }

    setSelectedNodeId(node.id);
    setDrawerOpen(true);
  }, [agentName, nodeDetails]);
          ...
          {!hideLhs && aiChatExpanded && (
            <div className="agent-builder__lhs agent-builder__lhs--expanded">
              <LHSDrawer
                ...
                composerAttachments={aiComposerAttachments}
                onRemoveComposerAttachment={handleRemoveComposerAttachment}
                onComposerFocus={armAiComposer}
                onComposerBlur={disarmAiComposerSoon}
                onClearComposerAttachments={handleClearComposerAttachments}
              />
            </div>
          )}
