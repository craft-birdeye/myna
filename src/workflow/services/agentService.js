...
const _SEED_TOOLS = [
  ...
  },
  {
    ...
    entities: ['review'],
    inputs: [{ name: 'reviewId', type: 'string' }, { name: 'tags', type: 'array' }],
  ...
  },
  {
    ...
    name: 'Send email',
    icon: 'mail',
    description: 'Sends an email alert to configured recipients when a review is marked as spam or needs human action.',
    category: 'Communication',
    modules: ['Reviews'],
  ...
  },
  {
    ...
    icon: 'rate_review',
    description: 'Posts a generated reply to the review on the source platform (Google, Facebook, Yelp, etc.).',
    category: 'Reviews',
    modules: ['Reviews'],
    products: ['healthcare', 'automotive', 'dental'],
    inputs: [
      { name: 'reviewId', type: 'string', description: 'Review ID' },
      { name: 'response', type: 'string', description: 'Generated review response text' },
    ],
    outputs: [{ name: 'status', type: 'string', description: 'posted | queued | failed' }],
  },
  {
    id: 'auto-follow-up',
    name: 'Auto follow-up',
    icon: 'replay',
    description: 'Sends automated follow-up messages or calls to visitors via text, email, or phone after inactivity, with configurable timing and business-hours-aware messaging.',
    category: 'Communication',
    modules: ['Front desk'],
    products: ['automotive', 'healthcare', 'dental'],
    hasDrawer: true,
  ...
  {
    id: 'escalation-notifier',
    name: 'Escalation notifier',
    icon: 'priority_high',
    description: 'Handles human escalation across voice, chat, text, and email — transfers calls, sends notifications to business users, and plays configurable messages during and outside business hours.',
    category: 'Communication',
    modules: ['Front desk'],
    products: ['automotive', 'healthcare', 'dental'],
    hasDrawer: true,
  ...
  },
  {
    ...
    name: 'Lead capture',
    icon: 'contact_page',
    description: 'Collects visitor contact details (name, phone, email) across voice, chat, and text channels with configurable mandatory fields and preview.',
    category: 'Sales',
    modules: ['Front desk'],
    products: ['automotive', 'healthcare', 'dental'],
    hasDrawer: true,
    inputs: [],
    outputs: [{ name: 'lead', type: 'object' }],
  },
  {
    id: 'lead-qualification',
    name: 'Lead qualification',
    icon: 'checklist',
    description: 'Runs a configurable qualification questionnaire to score and categorize incoming leads by intent, budget, and timeline.',
    category: 'Sales',
    modules: ['Front desk'],
    products: ['automotive', 'healthcare', 'dental'],
    hasDrawer: true,
    inputs: [],
    outputs: [{ name: 'qualificationScore', type: 'number' }, { name: 'qualified', type: 'boolean' }],
  },
  {
    id: 'voicemail-detection',
    name: 'Voicemail detection',
    icon: 'voicemail',
    description: 'Detects when an outbound call reaches voicemail and leaves a configurable message or hangs up based on settings.',
    category: 'Communication',
    modules: ['Front desk'],
    products: ['automotive', 'healthcare', 'dental'],
    hasDrawer: true,
    inputs: [],
    outputs: [{ name: 'leftVoicemail', type: 'boolean' }],
  },

  // ── Search AI tools ──────────────────────────────────────────────────────
  {
    id: 'query-fanout-generator',
    name: 'Query fanout generator',
    icon: 'call_split',
    description: 'Expands a tracked theme or keyword into a set of related sub-queries using semantic clustering, mirroring how AI platforms fan out a single question.',
    category: 'Search AI',
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
    inputs: [{ name: 'theme', type: 'string', description: 'Tracked theme or keyword' }],
    outputs: [{ name: 'subQueries', type: 'array', description: 'List of generated sub-queries' }],
  },
  {
    id: 'ai-platform-query-runner',
    name: 'AI platform query runner',
    icon: 'travel_explore',
    description: 'Executes a query against ChatGPT, Gemini, Perplexity, and Google AI Overviews, capturing whether and how the brand is mentioned in each response.',
    category: 'Search AI',
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
    inputs: [{ name: 'query', type: 'string' }, { name: 'platforms', type: 'array' }],
    outputs: [{ name: 'mentions', type: 'array', description: 'Per-platform mention and citation results' }],
  },
  {
    id: 'update-visibility-index',
    name: 'Update visibility index',
    icon: 'insights',
    description: 'Writes fanout query results back into the Visibility, Citations, and Rankings reports so dashboards reflect the latest run.',
    category: 'Search AI',
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
  ...
  {
    id: 'aeo-signal-checker',
    name: 'AEO signal checker',
    icon: 'fact_check',
    description: 'Scans published content for answer engine optimization signals — structured data, FAQ schema, and answer-friendly formatting — needed to be eligible for AI-generated answers.',
    category: 'Search AI',
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
    inputs: [{ name: 'contentUrl', type: 'string' }],
    outputs: [{ name: 'signals', type: 'object', description: 'Pass/fail per AEO signal' }],
  },
  {
    id: 'citation-validator',
    name: 'Citation validator',
    icon: 'link',
    description: 'Cross-checks the sources AI platforms cite for a page against the page’s actual published content to catch stale or inaccurate citations.',
    category: 'Search AI',
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
    inputs: [{ name: 'contentUrl', type: 'string' }],
    outputs: [{ name: 'validCitations', type: 'boolean' }],
  },
  {
    id: 'search-ai-alert',
    name: 'Send alert',
    icon: 'notifications_active',
    description: 'Notifies recipients inside Search AI when a monitored agent run produces a report worth reviewing.',
    category: 'Search AI',
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
    hasDrawer: true,
  ...
  {
    id: 'collect-user-prompt',
    name: 'Collect user prompt',
    icon: 'keyboard',
    description: 'Collects the prompt to run a fanout estimate against, typed manually or supplied via Run test.',
    category: 'Search AI',
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
    hasDrawer: true,
    inputs: [],
    outputs: [{ name: 'userPrompt', type: 'string' }],
  },
  {
    id: 'query-fanout-estimator',
    name: 'Query fanout estimator',
    icon: 'travel_explore',
    description: 'Estimates the fanout sub-queries AI engines would run to answer the collected prompt.',
    category: 'Search AI',
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
    inputs: [{ name: 'userPrompt', type: 'string' }],
    outputs: [{ name: 'fanoutEstimate', type: 'object' }],
  },
  {
    id: 'search-ai-recommendation',
    name: 'Send to AI recommendations',
    icon: 'lightbulb',
    description: "Publishes a finding as a new card in Search AI's Actions → Recommendations list.",
    category: 'Search AI',
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
    hasDrawer: true,
    inputs: [{ name: 'gap', type: 'object' }],
    outputs: [{ name: 'recommendationId', type: 'string' }],
  },
  {
    id: 'get-eligible-domains',
    name: 'Get all eligible domains',
    icon: 'travel_explore',
    description: 'Fetches every domain enrolled for domain health monitoring.',
    category: 'Search AI',
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
    inputs: [],
    outputs: [{ name: 'domains', type: 'array' }],
  },
  {
    ...
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
    inputs: [{ name: 'domains', type: 'array' }],
    outputs: [{ name: 'crawlStatus', type: 'string' }],
  },
  {
    id: 'website-health-analysis',
    name: 'Website Health Analysis',
    icon: 'health_and_safety',
    description: 'Analyzes crawled pages for facts, issues, scores, freshness, renderability, and recommendations.',
    category: 'Search AI',
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
    inputs: [{ name: 'domains', type: 'array' }],
    outputs: [
      { name: 'pageFacts', type: 'object' },
      { name: 'issues', type: 'array' },
      { name: 'scores', type: 'object' },
      { name: 'freshness', type: 'object' },
      { name: 'renderability', type: 'object' },
      { name: 'recommendations', type: 'array' },
    ],
  },
  {
    id: 'send-to-domain-health',
    name: 'Send to domain health',
    icon: 'send',
    description: 'Delivers the generated domain health report to the Domain health dashboard.',
    category: 'Search AI',
    modules: ['Search AI'],
    products: ['automotive', 'healthcare', 'dental'],
